# Merkezi bileşen dosyası videohelper+raghelper bileşenlerini kullanılan kısım
import streamlit as st
import videohelper
import raghelper

#sekmede görülen kısım
...
with tab_url:
    ...
    if submit_btn:
        ...
        st.divider()
        #2 aşamalı olacak
        #1.aşama: video->audio->text (videohelper) - zaman alır onun için spinner animasyonu koy
        #2.aşama: text(transkript)-> belek genişleme(raghelper)
...
#2.sekme arama sonuçlarında sohbet etkileşimi
with tab_search:
    ...
    with col_left:
        ...
        # yapılacaklar;
        # arama terimleri , kaç video gelsin, 
        # arama terimleri için sıralama kriterlerini belirleme 
        # ve arama butonu(aramaya hazır)
        ...
        #elde edile sonuçların depolandığı yer;
    ...
    #arama sonucunda gösterilen videolar
    with col_right:
        ...
        for i, video in enumerate(st.session_state.videos):
            st.info(f"Video No: {i+1}") #sayaç olauşturma
