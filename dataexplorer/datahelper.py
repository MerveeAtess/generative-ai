...
#summarize data-veri özeti oluşturmak
...
def summarize_csv(data_file): 
    ...
    data_summary["column_descriptions"] = pandas_agent.run("Verideki sütunları içeren bir tablo yap. Tabloda sütunların adları ve yanlarında kısaca içerdikleri bilgiye dair Türkçe bir açıklama yer alsın." \
    " Bunu bir tablo olarak ver.")

    data_summary["missing_values"] = pandas_agent.run("Bu veri kümesinde eksik veri var mı? Varsa kaç adet var?" \
    " Yanıtını 'Bu veri kümesinde X adet hücrede eksik veri var' şeklinde ver.")

    data_summary["duplicate_values"] = pandas_agent.run("Bu veri kümesinde mükerrer veri var mı? Varsa kaç adet var? " \
